...
from iod.data.datasets.coco import convert_to_coco_json
from iod.structures import Boxes, BoxMode, pairwise_iou
...
class COCOEvaluator(DatasetEvaluator):
    ...
    def _eval_predictions(self, tasks):
        ...
        if hasattr(self._metadata, "thing_dataset_id_to_contiguous_id"):
            ...
            category_index = reverse_id_mapping
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        for idx, name in enumerate(class_names):
            ...
            results_per_category.append(("{}".format(name), float(ap * 100))) # results_per_category：存放每类ap值
...
def _evaluate_predictions_on_coco(coco_gt, coco_results, iou_type, kpt_oks_sigmas=None):
    ...
    # voc_map_info_list = []
    # metadata = MetadataCatalog.get("coco_2014_minival")
    # # with open(metadata.json_file, 'r') as f: 
    # #     class_dict = json.load(f) 
    # category_index = metadata.thing_dataset_id_to_contiguous_id
    # print("category_index:",len(category_index))
    # for i in range(len(category_index)):
    #     stats,_ = coco_eval.summarize(catId=i)
    #     voc_map_info_list.append("{:80}: {}".format(category_index[i + 1], stats[1]))
