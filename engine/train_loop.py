...
from iod.utils.events import EventStorage
...
class SimpleTrainer(TrainerBase):
    ...
    def update_image_store(self, images):
        ...
        # print("更新后的图像存储器update_image_store:",self.image_store)
    ...
    def run_step(self):
        ...
        # 15_p_5.yaml  self.cfg.WG.TRAIN_WARP_AT_ITR_NO = 20 
        # self.cfg.WG.ENABLE = True if False, 更新元学习层时，cfg.WG.TRAIN_WARP = True，执行rcnn.py的get_warp_loss()函数
        if (self.iter + 1) % self.cfg.WG.TRAIN_WARP_AT_ITR_NO == 0 and self.cfg.WG.ENABLE:
            ...
            # print("images",images)
            # 原来代码是if not self.cfg.WG.USE_FEATURE_STORE:，现在改成if self.cfg.WG.USE_FEATURE_STORE
            if not self.cfg.WG.USE_FEATURE_STORE:
            # if self.cfg.WG.USE_FEATURE_STORE:
                for i in range(0, len(images), self.cfg.WG.BATCH_SIZE):
                    ...
                    self.optimizer.zero_grad()
                    warp_loss.backward()
                    # 设置任务损失梯度为0，仅更新元学习梯度层
                    for name, param in self.model.named_parameters():
                        if name not in self.cfg.WG.WARP_LAYERS and param.grad is not None:
                            param.grad.fill_(0)
                    self.optimizer.step()
            else:
                # cfg.WG.TRAIN_WARP = True，执行rcnn.py的get_warp_loss()函数 得到 warp_loss_dict: 'loss_cls': tensor(0.3627);'loss_box_reg':tensor(0.0191);'loss_dist_roi_head':tensor(0.0334)
                warp_loss_dict = self.model(images) 
                # print("..................")
                # print("images:",images)
                # print("..................")
                # print("warp_loss_dict:",warp_loss_dict)
        ...
        # # loss_dict: loss_cls、loss_box_reg、loss_dist_roi_head、loss_rpn_cls、loss_rpn_loc、loss_dist_backbone
        # 15_p_5.yaml: 新类检测损失：loss_cls、loss_box_reg、loss_rpn_cls、loss_rpn_loc + 旧类蒸馏损失：loss_dist_backbone(res4特征蒸馏)、loss_dist_roi_head(包括logit_distillation和anchor_delta_distillation)
        # 权重：主要还是新类的损失占比较大 6项损失
        loss_dict = self.model(data) # ft_15_p_5.yaml: 新类数据更新损失, 四项损失 旧类蒸馏损失和新类损失采用动态调节权重的方式进行训练
        # print("..................")
        # print("data:",data)
        # print("..................")
        ...
        if self.image_store is not None:
            ...
            # print("..................")
            # print("训练过程的self.image_store:",self.image_store)
        self._write_metrics(metrics_dict)
        # loss_cls = 0.1845；loss_box_reg = 0.0283；loss_dist_roi_head = 1.9722e-07；
        task_loss = sum(loss for loss in loss_dict.values())
        self._detect_anomaly(task_loss, loss_dict)
        # 15_p_5.yaml，image_store全是空队列,self.cfg.WG.ENABLE = True; ft_15_p_5.yaml: self.cfg.WG.ENABLE = False
        if self.cfg.WG.ENABLE:
            # Store the present data for future warp updates
            self.update_image_store(data) # 存放了新增类别的图像，其实就是存放的新类图像作为输入
            # Update task parameters on the task loss
            self.optimizer.zero_grad()
            task_loss.backward()
            for name, param in self.model.named_parameters():
                if name in self.cfg.WG.WARP_LAYERS:
                        param.grad.fill_(0) # 将模型中元学习层的参数梯度设置为0，更新任务层参数
            self.optimizer.step()
        # _C.FINETUNE.ENABLE = False；self.cfg.FINETUNE.ENABLE = False
            ...
            for name, param in self.model.named_parameters():
                if name in self.cfg.WG.WARP_LAYERS:
                        param.grad.fill_(0) # 将模型中元学习层的参数梯度设置为0
            self.optimizer.step()
        else:
            self.optimizer.zero_grad() # ft_15_p_5.yaml执行
            task_loss.backward()
            self.optimizer.step()
