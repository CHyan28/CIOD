...
from iod.layers.rotated_boxes import pairwise_iou_rotated
...
class RotatedBoxes(Boxes):
    ...
    def __getitem__(self, item: Union[int, slice, torch.BoolTensor]) -> "RotatedBoxes":
        """
        Returns:
            RotatedBoxes: Create a iOD :class:`RotatedBoxes` by indexing.
    ...
    def scale(self, scale_x: float, scale_y: float) -> None:
        ...
        # E(iOD) = (-sfx * c * w / 2, sfy * s * w / 2)
        # F(iOD) = (-sfx * s * h / 2, -sfy * c * h / 2)
        # The iOD width after scaling tranformation becomes:
        ...
        #
        # angle(iOD) = angle_yOx(O - F(iOD))
        #            = angle_yOx( (sfx * s * h / 2, sfy * c * h / 2) )
        #            = atan2(sfx * s * h / 2, sfy * c * h / 2)
        #            = atan2(sfx * s, sfy * c)
        #
        # For example,
        # when sfx == sfy, angle(iOD) == atan2(s, c) == angle(old)
