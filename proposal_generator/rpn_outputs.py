...
def rpn_losses(
    ...
    """
    pos_masks = gt_objectness_logits == 1 # 仅在有目标的位置进行定位预测，求定位损失
    localization_loss = smooth_l1_loss(
        pred_anchor_deltas[pos_masks], gt_anchor_deltas[pos_masks], smooth_l1_beta, reduction="sum"
    ) # 仅在前景的位置产生定位损失 localization_loss = tensor(48.0445, device='cuda:0', grad_fn=<SumBackward0>)
    #  
    ...
    objectness_loss = F.binary_cross_entropy_with_logits(
        ...
        reduction="sum",
    ) # objectness_loss = tensor(360.8431, device='cuda:0',
...
class RPNOutputs(object):
    def __init__(
        ...
        self.num_feature_maps = len(pred_objectness_logits) # self.num_feature_maps = 1
        self.num_images = len(images) # self.num_images = 2
    ...
    def _get_ground_truth(self):
        ...
        for image_size_i, anchors_i, gt_boxes_i in zip(self.image_sizes, anchors, self.gt_boxes):
            ...
            if len(gt_boxes_i) == 0:
                ...
                gt_anchor_deltas_i = torch.zeros_like(anchors_i.tensor) # 背景类
            else:
            ...
            gt_objectness_logits.append(gt_objectness_logits_i) # gt_objectness_logits[0].size()：torch.Size([26460]) 所有anchors的gt
            gt_anchor_deltas.append(gt_anchor_deltas_i) # gt_anchor_deltas[0].size()： torch.Size([26460, 4])
    ...
    def losses(self):
        ...
        gt_objectness_logits, gt_anchor_deltas = self._get_ground_truth() # 根据gt_boxes得到gt_objectness_logits和gt_anchor_deltas
        """
        gt_objectness_logits: list of N tensors. Tensor i is a vector whose length is the
        ...
        num_anchors_per_map = [np.prod(x.shape[1:]) for x in self.pred_objectness_logits] # 26460
        num_anchors_per_image = sum(num_anchors_per_map) # 26460
        ...
        num_pos_anchors = (gt_objectness_logits == 1).sum().item() # num_pos_anchors = 45  基类正anchors,新类的正anchors
        num_neg_anchors = (gt_objectness_logits == 0).sum().item() # num_neg_anchors = 467 基类的负anchors可能包含新类的正anchors
        ...
        assert gt_objectness_logits.shape[1] == num_anchors_per_image # 26460
        # Split to tuple of L tensors, each with shape (N, num_anchors_per_map)
        gt_objectness_logits = torch.split(gt_objectness_logits, num_anchors_per_map, dim=1) # torch.Size([2, 26460])
        # Concat from all feature maps
        gt_objectness_logits = cat([x.flatten() for x in gt_objectness_logits], dim=0) # torch.Size([52920])

        # Stack to: (N, num_anchors_per_image, B)
        gt_anchor_deltas = torch.stack(gt_anchor_deltas, dim=0) # gt_anchor_deltas[0]: torch.Size([26460, 4])
        ...
        gt_anchor_deltas = torch.split(gt_anchor_deltas, num_anchors_per_map, dim=1) # torch.Size([2, 26460, 4])
        # Concat from all feature maps
        gt_anchor_deltas = cat([x.reshape(-1, B) for x in gt_anchor_deltas], dim=0) # torch.Size([52920, 4])
        ...
        loss_cls = objectness_loss * normalizer  # cls: classification loss # loss_cls = tensor(0.7048, device='cuda:0', grad_fn=<MulBackward0>)
        loss_loc = localization_loss * normalizer  # loc: localization loss # loss_loc = tensor(0.0938, device='cuda:0', grad_fn=<MulBackward0>)
